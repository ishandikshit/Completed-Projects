...
def solveLogicQuestionRelation(intent, session):
    ...
    if relation in relations:
        ...
        speech_output = str(findMax())
    else:
        speech_output = str(findMin())
...
def solveLogicArithmeticRelativeMore(intent,session):
    ...
    person_db1['value'] = person_db2['value']+quantity
    putTable(person_db1)
    return build_response(session_attributes,
                          build_speechlet_response(card_title,
                          speech_output, reprompt_text,
...
def solveLogicArithmeticQuestionName(intent, session):
    ...
    if 'Item' in response:
        speech_output = str(name1)+" has "+str(response['value'])+" "+str(intent['slots']['Things']['value'])
    else:
...
def findMin():
    table = dynamodb.Table('FindLogic')
    result=table.scan()
    min =1000
    minloc =1000
    for i in range (0, result['Count']):
        if result['Items'][i]['value']<min:
            min=result['Items'][i]['value']
            minloc = i
...
def findMax():
    ...
    maxloc =-10
    for i in range (0, result['Count']):
        if result['Items'][i]['value']>max:
            max=result['Items'][i]['value']
            maxloc = i
